const { QuizSession, QuizQuestion, QuizSubmission, Pdf,  QuizGenerationUsage } = require("../models");
...
async function checkQuizGenerationLimit(userId, pdfId) {
  ...
  // Premium users have unlimited quiz generations
  if (user.is_premium) {
    return {
      allowed: true,
      usage: null,
  ...
  const usage = await QuizGenerationUsage.findOne({
    where: {
      user_id: userId,
      pdf_id: pdfId,
  ...
  if (usage && usage.generation_count >= 2) {
    return {
      allowed: false,
      usage,
    };
...
exports.generateQuiz = async(req, res) => {
    try {
        ...
        const limitCheck = await checkQuizGenerationLimit(userId, pdfId);

if (!limitCheck.allowed) {
  return res.status(403).json({
    message: "You've reached today's quiz limit. Upgrade to Premium for unlimited quiz generation.",
    limit: 2,
    used: limitCheck.usage.generation_count,
  });
}
...
if (!user.is_premium) {
  ...
  const [usage] = await QuizGenerationUsage.findOrCreate({
    where: {
      user_id: userId,
      pdf_id: pdfId,
      generation_date: today,
...
// Check quota BEFORE deleting the existing quiz
const limitCheck = await checkQuizGenerationLimit(userId, pdfId);
